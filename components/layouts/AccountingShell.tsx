...
import { Home, Building2, Euro, FolderOpen, Bell, BookOpen, ArrowLeft, MessageCircle, User, Receipt, ClipboardList, LogOut } from "lucide-react";
...
export default function AccountingShell({ children }: { children: React.ReactNode }) {
  ...
  const dashboardSidebar: { title: string; items: NavItem[] }[] = [
    { title: "", items: [{ label: "Nadzorna ploča", href: "/dashboard/accounting", icon: Home }, { label: "Svi SPV-ovi", href: "/dashboard/accounting/projekti", icon: Building2 }]},
    { title: "RAD", items: [{ label: "Otvoreni zahtjevi", href: "/dashboard/accounting/zahtjevi", icon: ClipboardList }]},
    { title: "TOK", items: [{ label: "Moji TOK-ovi", href: "/dashboard/accounting/tok", icon: MessageCircle }]},
    { title: "SUSTAV", items: [{ label: "Obavijesti", href: "/dashboard/accounting/obavijesti", icon: Bell }, { label: "Profil", href: "/dashboard/accounting/profil", icon: User }]},
  ];
  const spvSidebar: { title: string; items: NavItem[] }[] = [
    { title: "", items: [{ label: "Financije", href: `${spvBase}/financije`, icon: Euro }, { label: "Računi", href: `${spvBase}/racuni`, icon: Receipt }, { label: "Dokumenti", href: `${spvBase}/dokumenti`, icon: FolderOpen }, { label: "Zahtjevi", href: `${spvBase}/zahtjevi`, icon: ClipboardList }]},
    ...
    <div className="flex h-screen bg-[#f5f5f7]">
      <aside className="w-[232px] border-r border-black/[0.08] bg-white/60 backdrop-blur-xl flex flex-col">
        <div className="h-[52px] border-b border-black/[0.06] flex items-center px-4 gap-2.5">
          ...
          <span className="text-[9px] font-semibold text-black/25 bg-black/[0.04] px-1.5 py-0.5 rounded">KNJIG.</span>
        </div>
        <nav className="flex-1 overflow-y-auto px-2.5 py-3">
          {isInsideSpv && (<button onClick={() => router.push("/dashboard/accounting")} className="w-full flex items-center gap-2 px-3 py-2 mb-2.5 rounded-lg text-[12px] font-semibold text-black/50 hover:text-black hover:bg-black/[0.04] transition-all"><ArrowLeft size={14} strokeWidth={2.5} /><span>Natrag</span></button>)}
      ...
      <main className="flex-1 flex flex-col overflow-hidden">
        <header className="h-[52px] border-b border-black/[0.06] bg-white/80 backdrop-blur-xl flex items-center justify-between px-5"><div className="text-[13px] font-bold text-black">{isInsideSpv ? "SPV Financije" : "Knjigovodstvo"}</div><div className="flex items-center gap-2"><button onClick={() => router.push("/dashboard/notifications")} className="relative p-2 hover:bg-black/[0.04] rounded-lg transition-colors"><Bell size={16} strokeWidth={1.8} className="text-black/40" /></button><div className="relative"><button onClick={() => setUserMenuOpen(!userMenuOpen)} className="h-8 w-8 rounded-full bg-black flex items-center justify-center text-white text-[12px] font-bold hover:bg-black/80 transition-colors">K</button>{userMenuOpen && (<><div className="fixed inset-0 z-40" onClick={() => setUserMenuOpen(false)} /><div className="absolute right-0 top-full mt-2 w-52 bg-white/90 backdrop-blur-xl rounded-xl border border-black/[0.08] shadow-[0_8px_30px_rgba(0,0,0,0.08)] overflow-hidden z-50"><div className="p-3 border-b border-black/[0.06]"><div className="text-[12px] font-bold text-black">Knjigovodstvo</div></div><div className="p-1"><button onClick={handleLogout} className="w-full text-left px-3 py-1.5 rounded-lg text-[12px] font-medium text-red-500/80 hover:bg-red-50">Odjava</button></div></div></>)}</div></div></header>
