...
import { Home, FolderOpen, Euro, Bell, BookOpen, Landmark, ArrowLeft, MessageCircle, User, Archive, ClipboardCheck, LogOut } from "lucide-react";
    ...
    <div className="flex h-screen bg-[#f5f5f7]">
      <aside className="w-[232px] border-r border-black/[0.08] bg-white/60 backdrop-blur-xl flex flex-col">
        <div className="h-[52px] border-b border-black/[0.06] flex items-center px-4 gap-2.5">
          ...
          <span className="text-[9px] font-semibold text-black/25 bg-black/[0.04] px-1.5 py-0.5 rounded">BANKA</span>
        </div>
        <nav className="flex-1 overflow-y-auto px-2.5 py-3">
          {isInsideSpv && (<button onClick={() => router.push("/dashboard/bank")} className="w-full flex items-center gap-2 px-3 py-2 mb-2.5 rounded-lg text-[12px] font-semibold text-black/50 hover:text-black hover:bg-black/[0.04] transition-all"><ArrowLeft size={14} strokeWidth={2.5} /><span>Natrag</span></button>)}
      ...
      <main className="flex-1 flex flex-col overflow-hidden">
        <header className="h-[52px] border-b border-black/[0.06] bg-white/80 backdrop-blur-xl flex items-center justify-between px-5"><div className="text-[13px] font-bold text-black">{isInsideSpv ? "SPV Evaluacija" : "Banka"}</div><div className="flex items-center gap-2"><button onClick={() => router.push("/dashboard/notifications")} className="relative p-2 hover:bg-black/[0.04] rounded-lg transition-colors"><Bell size={16} strokeWidth={1.8} className="text-black/40" /></button><div className="relative"><button onClick={() => setUserMenuOpen(!userMenuOpen)} className="h-8 w-8 rounded-full bg-black flex items-center justify-center text-white text-[12px] font-bold hover:bg-black/80 transition-colors">B</button>{userMenuOpen && (<><div className="fixed inset-0 z-40" onClick={() => setUserMenuOpen(false)} /><div className="absolute right-0 top-full mt-2 w-52 bg-white/90 backdrop-blur-xl rounded-xl border border-black/[0.08] shadow-[0_8px_30px_rgba(0,0,0,0.08)] overflow-hidden z-50"><div className="p-3 border-b border-black/[0.06]"><div className="text-[12px] font-bold text-black">Banka</div></div><div className="p-1"><button onClick={handleLogout} className="w-full text-left px-3 py-1.5 rounded-lg text-[12px] font-medium text-red-500/80 hover:bg-red-50">Odjava</button></div></div></>)}</div></div></header>
