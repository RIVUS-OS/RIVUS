...
export default function CoreShell({ children }: { children: React.ReactNode }) {
  ...
  const coreDooSidebar: { title: string; items: NavItem[] }[] = [
    { title: "", items: [{ label: "Nadzorna ploča", href: "/dashboard/core-company", icon: Home }]},
    ...
    { title: "POREZNO & IZVJEŠTAJI", items: [
  ...
  const controlRoomSidebar: { title: string; items: NavItem[] }[] = [
    { title: "", items: [{ label: "Dashboard", href: "/dashboard/core", icon: Home }]},
    ...
    { title: "NADZOR", items: [
      ...
      { label: "Knjig.", href: "/dashboard/core/knjigovodje-nadzor", icon: UserCog },
        ...
        <nav className="flex-1 overflow-y-auto px-3 py-4">
          ...
          {activeSidebar.map((section, idx) => (
            <div key={idx} className="mb-5">
              {section.title && (
                <div className="px-3 mb-2 text-[11px] font-semibold tracking-[0.08em] text-[#6E6E73]/60 uppercase">{section.title}</div>
              )}
      ...
      <main className="flex-1 flex flex-col overflow-hidden">
        ...
        <header className="h-[56px] border-b border-[#E8E8EC] bg-white flex items-center justify-between px-6">
          ...
          <div className="flex items-center gap-3">
            <div className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-[#F5F5F7] text-[12px] text-[#8E8E93]">
