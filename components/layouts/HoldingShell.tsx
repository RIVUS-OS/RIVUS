...
const holdingNav = [
  { label: "Pregled",      href: "/dashboard/holding" },
  { label: "Portfolio",    href: "/dashboard/holding/portfolio" },
  { label: "Financije",    href: "/dashboard/holding/financije" },
  { label: "Rizik",        href: "/dashboard/holding/rizik" },
  { label: "Izvještaji",   href: "/dashboard/holding/izvjestaji" },
  { label: "Tok",          href: "/dashboard/holding/tok" },
  { label: "Profil",       href: "/dashboard/holding/profil" },
];
    ...
    <div className="flex h-screen bg-[#F7F7F8]"
      ...
      <aside className="w-[240px] border-r border-[#E8E8EC] bg-white flex flex-col flex-shrink-0">
        <div className="h-[56px] border-b border-[#E8E8EC] flex items-center px-5 gap-2.5">
          ...
          <div>
            ...
            <span className="text-[9px] font-bold text-[#8E8E93] bg-[#F5F5F7] px-2 py-0.5 rounded ml-2">HOLD.</span>
          </div>
        </div>
        <nav className="flex-1 overflow-y-auto px-3 py-4">
          <div className="space-y-0.5">
            {holdingNav.map((item) => {
              const exact = item.href === "/dashboard/holding";
        ...
        <div className="border-t border-[#E8E8EC] px-4 py-3">
          <div className="flex items-center gap-3 mb-3">
            <div className="h-8 w-8 rounded-full bg-[#2563EB] flex items-center justify-center text-white text-[13px] font-bold flex-shrink-0">H</div>
            <div className="min-w-0">
              <div className="text-[13px] font-semibold text-[#0B0B0C] truncate">Holding</div>
              <div className="text-[11px] text-[#8E8E93]">RIVUS Holding</div>
            </div>
          </div>
      ...
      <main className="flex-1 flex flex-col overflow-hidden">
        <header className="h-[56px] border-b border-[#E8E8EC] bg-white flex items-center justify-between px-6 flex-shrink-0">
          <div className="text-[14px] font-semibold text-[#3C3C43]">Holding</div>
          <div className="flex items-center gap-3">
            ...
            <div className="relative">
              <button onClick={() => setUserMenuOpen(!userMenuOpen)}
                className="h-8 w-8 rounded-full bg-[#2563EB] flex items-center justify-center text-white text-[12px] font-bold">H</button>
              {userMenuOpen && (<>
                ...
                <div className="absolute right-0 top-full mt-2 w-44 bg-white rounded-xl border border-[#E8E8EC] shadow-lg overflow-hidden z-50">
                  <div className="p-1">
                    <button onClick={() => { router.push("/dashboard/holding/profil"); setUserMenuOpen(false); }}
