...
const statusColors: Record<string, string> = {
  registriran: "bg-blue-100 text-blue-700",
  placen: "bg-green-100 text-green-700",
  dospjelo: "bg-red-100 text-red-700",
  storno: "bg-gray-100 text-gray-500",
};

export default function CoreCompanyPrimljeniRacuniPage() {
  ...
  useEffect(() => {
    if (!permLoading && allowed) {
      logAudit({ action: "CORE_COMPANY_RECEIVED_INVOICES_VIEW", entity_type: "core_invoice", details: { context: "core_doo_primljeni" } });
    }
  }, [permLoading, allowed]);
  ...
  const racuni = [
    { id: "PR-2026-001", date: "2026-02-28", dobavljac: "MIT Knjigovodstvo", opis: "Knjigovodstvene usluge — veljaca", iznos: "€ 300,00", pdv: "€ 75,00", ukupno: "€ 375,00", status: "placen" },
    { id: "PR-2026-002", date: "2026-02-15", dobavljac: "Vercel Inc.", opis: "Hosting — veljaca 2026", iznos: "€ 20,00", pdv: "€ 0,00", ukupno: "€ 20,00", status: "placen" },
    { id: "PR-2026-003", date: "2026-02-15", dobavljac: "Supabase Inc.", opis: "Database Pro — veljaca 2026", iznos: "€ 25,00", pdv: "€ 0,00", ukupno: "€ 25,00", status: "placen" },
    { id: "PR-2026-004", date: "2026-03-01", dobavljac: "MIT Knjigovodstvo", opis: "Knjigovodstvene usluge — ozujak", iznos: "€ 300,00", pdv: "€ 75,00", ukupno: "€ 375,00", status: "registriran" },
    { id: "PR-2026-005", date: "2026-02-01", dobavljac: "Odvjetnik", opis: "Pravno savjetovanje — NDA", iznos: "€ 500,00", pdv: "€ 125,00", ukupno: "€ 625,00", status: "dospjelo" },
  ];

  return (
    <div className="space-y-6">
      {isSafe && <div className="p-3 rounded-xl bg-amber-50 border border-amber-300 text-[13px] text-amber-800 font-medium">Sustav u Safe Mode — registracija racuna onemogucena.</div>}
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-[22px] font-bold text-black">CORE d.o.o. — Primljeni racuni</h1>
          <p className="text-[13px] text-black/50 mt-0.5">{racuni.length} racuna</p>
        </div>
        <button disabled={writeDisabled} className={`px-4 py-2 rounded-lg text-[13px] font-semibold ${writeDisabled ? "bg-gray-100 text-gray-400 cursor-not-allowed" : "bg-black text-white hover:bg-gray-800"}`}>+ Registriraj racun</button>
      </div>

      <div className="p-3 rounded-xl bg-blue-50 border border-blue-200 text-[12px] text-blue-700">Storno umjesto brisanja (A10-K1). Period Lock gate blokira write na zakljucanom periodu.</div>
      ...
      <div className="bg-white rounded-xl border border-gray-200 overflow-x-auto">
        <table className="w-full text-[12px]">
          ...
          <tbody>{racuni.map(r => (
            <tr key={r.id} className={`border-b border-gray-50 hover:bg-gray-50 ${r.status === "dospjelo" ? "bg-red-50/30" : ""}`}>
              <td className="px-3 py-2.5 font-bold">{r.id}</td>
              <td className="px-3 py-2.5 text-black/70">{r.date}</td>
              <td className="px-3 py-2.5 text-black/70">{r.dobavljac}</td>
