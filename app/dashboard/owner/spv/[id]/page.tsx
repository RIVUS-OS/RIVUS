...
export default function OwnerSpvPage() {
  ...
  const unpaidIssued = issued.filter(i => { const s = i.status as string; return s !== "plaćen" && s !== "storniran"; });
  const openTasks = tasks.filter(t => (t.status as string) !== "završen");
  const pendingDecisions = decisions.filter(d => (d.status as string) === "na_čekanju");
      ...
      <div className="flex flex-wrap gap-1 border-b border-gray-200">
        {[
          ...
          <button key={tab.href} onClick={() => router.push(tab.href)}
            className={`px-3 py-2 text-[12px] font-medium rounded-t-lg ${tab.active ? "bg-white border border-b-white border-gray-200 text-black -mb-px" : "text-black/50 hover:text-black hover:bg-gray-50"}`}>{tab.label}</button>
        ))}
      </div>
          ...
          <div className="bg-white rounded-xl border border-gray-200 p-5">
            ...
            {accountant ? (
              <div className="text-[12px]">
                ...
                <div className="text-black/50 mt-1">{accountant.contact}</div>
                <div className="text-black/50">{formatEur(accountant.pricePerMonth)} / mj</div>
              </div>
            ) : <div className="p-3 rounded-lg bg-red-50 border border-red-200 text-[12px] text-red-700 font-medium">Nema knjigovodje!</div>}
          </div>
        </div>
        <div className="space-y-4">
          <div className="bg-white rounded-xl border border-gray-200 p-5">
