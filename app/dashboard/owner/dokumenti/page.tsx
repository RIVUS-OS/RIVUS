...
export default function OwnerDokumentiPage() {
  ...
  useEffect(() => { if (!permLoading && allowed) logAudit({ action: "OWNER_DOKUMENTI_VIEW", entity_type: "page", details: {} }); }, [permLoading, allowed]);
  ...
  const allDocs = spvs.flatMap((p) =>
    _docsAll.filter((x) => x.spvId === p.id)
  );
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-[13px] text-black/50 mt-0.5">
          {allDocs.length} ukupno | {missing.length} nedostaje
        </p>
      </div>
      {missing.length > 0 && (
        <div className="p-4 rounded-xl bg-red-50 border-2 border-red-200">
          <div className="text-[14px] font-bold text-red-700 mb-2">
            Nedostajuci mandatory dokumenti ({missing.length})
          </div>
          {missing.map((d) => (
            <div
              key={d.id}
              className="flex items-center justify-between p-2 rounded-lg bg-red-100/50 mb-1 text-[12px]"
            >
              <span className="text-red-700 font-medium">{d.name}</span>
              <span className="text-red-600">
                {spvMap.get(d.spvId) || d.spvId} | {d.type}
              </span>
            </div>
          ))}
        </div>
              ...
              return (
                ...
                >
                  ...
                  <td className="px-3 py-2.5 text-black/70">
                    {spvMap.get(d.spvId) || d.spvId}
                  </td>
                  <td className="px-3 py-2.5 text-black/50">{d.type}</td>
                  <td className="px-3 py-2.5 text-center">
                    {d.mandatory ? (
                      <span className="text-red-600 font-bold">DA</span>
                    ) : (
                      "-"
                    )}
                  </td>
