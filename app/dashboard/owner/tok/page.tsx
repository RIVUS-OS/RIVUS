...
export default function OwnerTokPage() {
  ...
  const open = allTok.filter(t => t.status === "otvoren" || t.status === "u_tijeku" || t.status === "eskaliran");
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-[13px] text-black/50 mt-0.5">{allTok.length} ukupno | {open.length} otvorenih | {slaBreached.length} SLA probijenih</p>
      </div>
      <div className="grid grid-cols-3 gap-3">
        {[
          { label: "Otvoreni", value: open.length, color: "text-blue-600" },
          { label: "SLA probijeni", value: slaBreached.length, color: slaBreached.length > 0 ? "text-red-600" : "text-green-600" },
        ...
        <table className="w-full text-[12px]">
          ...
          <tbody>{allTok.map(t => (
            <tr key={t.id} className={`border-b border-gray-50 hover:bg-gray-50 ${t.slaBreached ? "bg-red-50/30" : ""}`}>
              <td className="px-3 py-2.5 font-bold text-black">{t.id}</td>
