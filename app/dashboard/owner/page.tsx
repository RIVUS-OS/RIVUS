  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap gap-1 border-b border-gray-200">
        {[
          ...
          <button key={tab.href} onClick={() => router.push(tab.href)}
            className={`px-3 py-2 text-[12px] font-medium rounded-t-lg ${tab.active ? "bg-white border border-b-white border-gray-200 text-black -mb-px" : "text-black/50 hover:text-black hover:bg-gray-50"}`}>
            {tab.label}
          </button>
        ))}
      </div>
      <div className="grid grid-cols-3 sm:grid-cols-6 gap-3">
        {[
          { label: "Racuni", value: issued.length, sub: formatEur(issued.reduce((acc, i) => acc + i.totalAmount, 0)), alert: false },
          { label: "Nenaplaceno", value: unpaidIssued.length, sub: formatEur(unpaidIssued.reduce((acc, i) => acc + i.totalAmount, 0)), alert: unpaidIssued.length > 0 },
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="space-y-4">
          ...
          <div className="bg-white rounded-xl border border-gray-200 p-5">
            ...
            <div className="p-3 rounded-lg bg-yellow-50 border border-yellow-200 text-[12px] text-yellow-700 font-medium">Podaci u pripremi</div>
          </div>
        </div>
        <div className="space-y-4">
          <div className="bg-white rounded-xl border border-gray-200 p-5">
