...
export default function NetoKvartalnoPage() {
  ...
  for (let i = 0; i < pnlMonths.length; i += 3) { const c = pnlMonths.slice(i,i+3); const rev = c.reduce((s,m)=>s+m.revenue,0); const exp = c.reduce((s,m)=>s+m.expenses,0); quarters.push({q:`Q${Math.floor(i/3)+1}`,rev,exp,net:rev-exp}); }
